import {
	...
	transformGradeData
} from '../src/lib/utils/csvParser.ts';
...
if (fs.existsSync(outputFile)) {
	...
	const shouldRebuild = sourceFiles.some((file) => fs.statSync(file).mtimeMs > outputTime);
	if (!shouldRebuild) {
...
async function processData() {
	try {
		...
		fs.writeFileSync(path.join(outputDir, 'combined-data.json'), JSON.stringify(combinedData));
		...
		console.log(
			`Generated ${departments.length} departments, ${years.length} years, ${instructors.length} instructors`
		);
	} catch (error) {
