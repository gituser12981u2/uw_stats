...
export function combineData(
    ...
    const combinedEvaluations = new Map<string, {
        param: EvalParam;
    ...
    evalParamsData.slice(0, 10).forEach(param => {
        ...
        if (median) {
            matchedCount++;
            if (matchedCount <= 3) {
                console.log(`EvalID ${param.EvalID}: ${param.CourseAbbrev} ${param.CourseNumber} has median data (MedianGlobal: ${median.MedianGlobal})`);
            }
        }
    });
    ...
    gradesData.forEach((grade, index) => {
        ...
        for (const { term, year } of possibleTermYears) {
            ...
            for (const key of searchKeys) {
                ...
                if (matchedEval) {
                    if (index < 5) {
                        console.log(`✓ Matched grade ${grade.Course_Number} ${grade.Primary_Instructor} using key: ${key}`);
                        console.log(`  -> EvalID: ${matchedEval.param.EvalID}, MedianGlobal: ${matchedEval.median?.MedianGlobal || 'N/A'}`);
                    }
                    break;
                }
            }
    ...
    console.log(`Grades matched to eval params: ${successfulMatches} (${(successfulMatches / gradesData.length * 100).toFixed(1)}%)`);
    console.log(`Grades with actual rating data: ${gradesWithEvalData} (${(gradesWithEvalData / gradesData.length * 100).toFixed(1)}%)`);
...
export function calculateStats(data: CombinedCourseData[]): CourseStats {
    ...
    return {
        ...
        avgGPA: validGPA.length > 0 ?
            validGPA.reduce((sum, d) => sum + d.Average_GPA, 0) / validGPA.length : 0,
        avgRating: validRating.length > 0 ?
            validRating.reduce((sum, d) => sum + (d.evalMedian?.MedianGlobal || 0), 0) / validRating.length : 0,
